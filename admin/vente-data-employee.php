...
if (isset($_POST['saveSell'])) {
    ...
    if ($_FILES['image']['size'] > 0) {
        ...
        if ($imgFileTypes != 'jpg' && $imgFileTypes != 'jpeg' && $imgFileTypes != 'png') {
            redirect('vente-employee.php', 'Seulement les images JPG, JPEG, PNG sont accepté');
        }
    ...
    if ($result) {
        ...
        redirect('vente-employee.php', 'Vente Ajouté');
    } else {
        redirect('vente-employee.php', 'Une errur est survenue.');
    }
}
...
if(isset($_POST['updateSell'])){
    ...
    if ($result) {
        move_uploaded_file($_FILES['image']['tmp_name'], $path.$filename);
        redirect('vente-edit-employee.php?id='.$venteId, 'Vente Modifié');
    } else {
        redirect('vente-edit-employee.php?id='.$venteId, 'Une errur est survenue.');
    }
